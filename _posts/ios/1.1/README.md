...
The provided zip archive contains the binary framework and a sample appliction.
...
Add a new Run Script Phase in your target's Build Phases. This new build phase should come after the Embed Frameworks phase. Copy the following to the shell text area:
...
When the Player sends the callback: `(void)playerStateDidChange:(USUstreamPlayer *)player` it indicates that the state of the Player has changed so that you may choose to act on it. The states are listed in the `USPlayerState` enum.
...
When the Player sends the callback: `(void)playerRequiresPassword:(USUstreamPlayer *)player` it waits for a password belonging to the locked content. You can provide the password using `continueWithPassword:(NSString *)password`
...
When the Player sends the callback `(void)playerRequiresAgeConfirmation:(USUstreamPlayer *)player ageRequired:(NSUInteger)age` or the callback: `(void)playerRequiresBirthdate:(USUstreamPlayer *)player` you need to ask the user's birth date without hints to the required age. If the user-specified age is not less than the required age, you can continue the playback with `continueWithAgeConfirmed`.
